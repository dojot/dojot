...
log "INFO" "Cluster wide pre-upgrade hook invoked by orchestration tool."

if [ -f ${baseDir}/bin/internal/cluster-upgrade-pre-app.sh  ] ; then source ${baseDir}/bin/internal/cluster-upgrade-pre-app.sh  "${baseDir}" "${tempDir}" ; fi
