...
VERNE_K2V_BRIDGE_KAFKA_HOSTS=${KAFKA_HOSTS:-"kafka-server:9092"}
VERNE_K2V_BRIDGE_DATA_BROKER_HOST=${DATA_BROKER:-"data-broker:80"}

# check if kafka is up
kafka_hosts=$(echo $VERNE_K2V_BRIDGE_KAFKA_HOSTS | tr "," "\n")
...
IFS=':' read -ra data_broker <<< $VERNE_K2V_BRIDGE_DATA_BROKER_HOST
...
/opt/k2v_bridge/bin/scripts_tls/ejbca_client.sh
