...
### HTTPS request to Authorization Default Mode(fingerprint):
...
### HTTPS request to Authorization CN Mode:

This authorization mode expects a certificate with a CN in the pattern <tenant>:<device>. Must use when the stakeholder wants to use an external certificate generated by the stakeholder without the dojot security feature. 

The HTTPs request for this mode follows the same pattern shown in the previous example.

### HTTPS request with basic-auth (Unsecure Mode):

```HTTP
  POST v1/unsecure/incoming-messages/create-many \
    -H 'Authorization: Basic YmFzaWNAYTExZjIyOlApcDRtKjVSY1dOenA7Pw==' \
    -H 'Content-Type: application/json' \
    -d '[
      {
        "ts": "2023-01-09T17:37:01.683Z",
        "data": {
          "temperature": 25.79,
          "heartRate": 70,
          "respiratoryRate": 40,
...
Note: Basic-auth credentials will be provided by the dojot basic-auth service.
