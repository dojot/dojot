...
__ATTENTION__  It is important to note that in services that use node.js and its tls library, the certificates, including the crl, are not changed after the service starts even if the files are exchanged. Therefore, it is necessary to have some solution in the deployment, such as a maximum lifetime of a container or pod.
