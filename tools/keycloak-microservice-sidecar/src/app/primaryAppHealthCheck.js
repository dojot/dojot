...
async function check(url, timeout, logger) {
  return new Promise((resolve) => {
    const req = http.get(url, { timeout }, (res) => {
      ...
      logger.debug('PrimaryAppHealthCheck - Connection error', ex);
      resolve(false);
    }).on('timeout', () => {
      ...
      // req.destroy(new Error('PrimaryAppHealthCheck - Connection timeout'));
...
module.exports = PrimaryHealthCheck;
