...
const logger = new Logger('device-auth:HTTPServer');
const {
...
class HTTPServer {
   ...
   *          with register service 'device-auth-server'} serviceState
  ...
  init(express) {
    ...
    this.httpServer.on('listening', () => {
      ...
      this.serviceState.signalReady('device-auth-server');
    });
    this.httpServer.on('close', () => {
      this.serviceState.signalNotReady('device-auth-server');
    });
