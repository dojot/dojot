...
const logger = new Logger('device-auth:DB');

const config = unflatten(getConfig('DEVICE_AUTH'));
...
class MongoClient {
   ...
   *          with register service 'device-auth-db'} serviceState
  ...
  init() {
    try {
      ...
      mongoose.connection.on('connected', () => {
        ...
        this.serviceState.signalReady('device-auth-db');
      });
      ...
      mongoose.connection.on('disconnected', () => {
        ...
        this.serviceState.signalNotReady('device-auth-db');
        if (!this.initialized) {
