...
const { sync } = getConfig('DEVICE_AUTH');

const logger = new Logger('device-auth:sync');
...
class SyncLoader {
  ...
  async load() {
    ...
    for (const tenant of credentilsTenants) {
      // Sync and load
      try {
        if (tenants.includes(tenant)) await this.loadDevices(tenant);
        else await this.basicCredentials.removeAllFromTenant(tenant);
      } catch (error) {
  ...
  async loadDevices(tenant) {
    ...
    for (const device of credentilsDevices) {
      if (!devices.includes(device)) await this.basicCredentials.remove(tenant, device);
    }
  }
}
