...
const tenantManagerEventEngine = container.resolve('tenantManagerEventEngine');
...
setTimeout(() => { secretFileHandler.handle('keycloak.client.secret', '/secrets/').then(async () => {
  ...
  tenantManagerEventEngine.start();

  // initializes all the tenants and the keycloak sessions
  ...
  stateManager.registerShutdownHandler(async () => {
    ...
    await tenantManagerEventEngine.stop();
    return Promise.resolve(true);
  });
