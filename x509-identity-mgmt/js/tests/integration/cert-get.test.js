...
describe('X509 Certificates - GET integrations', () => {
  it('should get a certificate',
    () => {
      ...
      return req.get(`/v1/x509-certificates/${fingerprint}?fields=fingerprint`)
  ...
  it('should get a list of certificates',
    () => {
      ...
      return req.get('/v1/x509-certificates?fields=fingerprint')
        .set('Authorization', `Bearer ${token}`)
        .send()
        .expect(200)
        .then((res) => {
          expect(res.body).toEqual({
            paging: {
              ...
              itemLimitPerPage: 25,
              totalItems: 3,
              pages: [{ number: 1, url: '/v1/x509-certificates?fields=fingerprint&page=1&limit=25' }],
            },
  ...
  it('should limit the listing to 1 element per page',
    () => {
      ...
      return req.get('/v1/x509-certificates?limit=0;fields=fingerprint')
        .set('Authorization', `Bearer ${token}`)
        .send()
        .expect(200)
        .then((res) => {
          expect(res.body).toEqual({
            paging: {
              ...
              pages: [
                {
                  number: 1,
                  url: '/v1/x509-certificates?limit=1&page=1',
                },
                {
                  number: 2,
                  url: '/v1/x509-certificates?limit=1&page=2',
                },
                {
                  number: 3,
                  url: '/v1/x509-certificates?limit=1&page=3',
                },
              ],
            },
            certificates: results,
