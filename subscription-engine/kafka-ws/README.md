...
redis.connect_timeout | Redis Connection timeout in milliseconds (It is recommended to be greater than or equal to `server.connection.max.life.time`) | 7200000 | integer (ms) | KAFKA_WS_REDIS_CONNECT__TIMEOUT
...
server.connection.max.life.time| Maximum lifetime of a connection in seconds (-1 to disable) - It is recommended to be less than or equal to `redis.connect_timeout` | 7200 | integer (sec) | KAFKA_WS_SERVER_CONNECTION_MAX_LIFE_TIME
