...
func NewHTTPClient(config *HTTPConfig) (*httpClient, error) {
	...
	client := &httpClient{
		...
		OrganizationTag:  config.OrganizationTag,
...
type OrganizationBatchs = map[string][]telegraf.Metric
...
func (c *httpClient) Write(ctx context.Context, metrics []telegraf.Metric) error {
	...
	batches := make(map[string]OrganizationBatchs)

	if c.BucketTag == "" && c.OrganizationTag == "" {
		err := c.writeBatch(ctx, c.Organization, c.Bucket, metrics)
		if err != nil {
		...
		for _, metric := range metrics {
			// Organization handle
			organization, ok := metric.GetTag(c.OrganizationTag)
			if !ok {
				organization = c.Organization
			}

			if _, ok := batches[organization]; !ok {
				batches[organization] = make(OrganizationBatchs, 0)
			}
			...
			if _, ok := batches[organization][bucket]; !ok {
				batches[organization][bucket] = make([]telegraf.Metric, 0)
			}
			...
			batches[organization][bucket] = append(batches[organization][bucket], metric)
		}
	}

	for org, orgBatches := range batches {
		for bucket, batch := range orgBatches {
			err := c.writeBatch(ctx, org, bucket, batch)
			if err != nil {
				if err, ok := err.(*APIError); ok {
					if err.StatusCode == http.StatusRequestEntityTooLarge {
...
func (c *httpClient) splitAndWriteBatch(ctx context.Context, organization string, bucket string, metrics []telegraf.Metric) error {
	...
	if err := c.writeBatch(ctx, organization, bucket, metrics[:midpoint]); err != nil {
		return err
	}
